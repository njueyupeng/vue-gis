...
async function bundle(options?: BuildOptions) {
  await build({
    outfile: `${cwd()}/dist/es/vue-gis.esm.js`,
    bundle: true,
    entryPoints: [`${cwd()}/src/packages/vue-gis.ts`],
    plugins: [vue()],
    loader: { '.png': 'dataurl' },
    external: ['vue', 'vue-gis/*', '@vue/*'],
    format: 'esm',
    minify: true,
    ...options,
  })
...
async function combineCss() {
  ...
  await Promise.all([
    fs.promises.writeFile(
      `${cwd()}/dist/es/vue-gis.esm.css`,
      content
    ),
    fs.promises.writeFile(
      `${cwd()}/dist/lib/vue-gis.umd.css`,
      content
    ),
  ])

  const name = 'vue-gis.min.css'
  await Promise.all([
    fs.promises.rename(
      `${cwd()}/dist/es/vue-gis.esm.css`,
      `${cwd()}/dist/es/${name}`
